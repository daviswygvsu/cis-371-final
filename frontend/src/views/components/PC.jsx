...
function PC (props ){
    ...
    return(
        <tr>
            ...
            <td>{props.pc.xp}</td>
            <td>{props.pc.gp}</td>
            <button type = 'button' onClick = { () => { navigate(`/pcs/edit/${props.pc.id}`) } }>Edit</button><button type = 'button' className = 'dbutton' onClick = { () => { fetchDestroy( props.pc.id, props.update ) } }>Destroy</button>
        </tr>
    )
}
